...
export async function POST() {
  try {
    ...
    // Group responses by supplier for per-supplier analysis
    const bySupplier = new Map<
      string,
      { name: string; texts: string[] }
    >();
    ...
    // Process each supplier in batches of 50 responses per AI call
    for (const [supplierId, { texts }] of bySupplier) {
      ...
      // Aggregate topics: merge by name, sum mentions
      ...
      const sentimentShift = -(negativeShift * 100 - 50); // Normalize: positive = good

      // Upsert into worker_voice_trends
      await db
    ...
    // Also do a global analysis (all responses combined)
