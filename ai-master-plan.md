...
| **Automatic Case Clustering**   | ⏳ Planned    | Group 8,277 messages via embedding and cosine similarity. Detect systemic, cross-factory patterns (e.g., regional wage withholding).              | **Local Ollama** (`bge-m3` for embeddings + `qwen3:4b` for labeling). pgvector for similarity search. Store in `case_embeddings` + `case_clusters` tables. |
